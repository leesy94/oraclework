...
INSERT INTO TB_PROSTOCK
    VALUES(SEQ_TCODE.NEXTVAL,200,SYSDATE,7,1200000);

//TB_PROSALE 출고테이블에 INSERT 이벤트 발생
...
DECLARE
    SCOUNT NUMBER;
BEGIN
    SELECT STOCK_QUANT INTO SCOUNT FROM TB_PRODUCT 
    WHERE PCODE = :NEW.PCODE; 
    ...
    IF SCOUNT >= :NEW.SALE_COUNT
        ...
        WHERE PCODE = :NEW.PCODE; 
    ELSE
        RAISE_APPLICATION_ERROR(-20000,'재고수량 부족');
    END IF;
END;
/
